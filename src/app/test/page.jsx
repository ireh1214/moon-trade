...
const ItemList = () => {
  const [shopData, setShopData] = useState({}); // API 데이터를 저장할 상태 변수
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://open.api.nexon.com/mabinogi/v1/npcshop/list?npc_name=%EC%83%81%EC%9D%B8%20%EB%88%84%EB%88%84&server_name=%EB%A5%98%ED%8A%B8&channel=37",
          {
            headers: {
  ...
  const filteredTabs = shopData.shop.filter((tab) => tab.tab_name === "주머니");
